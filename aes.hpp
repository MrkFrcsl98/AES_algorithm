...
static constexpr std::array<uint8_t, 11> rcon = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36};

// AES MixColumns matrix
static constexpr std::array<std::array<uint8_t, 4>, 4> mix_columns = {{{0x02, 0x03, 0x01, 0x01}, {0x01, 0x02, 0x03, 0x01}, {0x01, 0x01, 0x02, 0x03}, {0x03, 0x01, 0x01, 0x02}}};

// AES Inverse MixColumns matrix
static constexpr std::array<std::array<uint8_t, 4>, 4> inv_mix_columns = {{{0x0E, 0x0B, 0x0D, 0x09}, {0x09, 0x0E, 0x0B, 0x0D}, {0x0D, 0x09, 0x0E, 0x0B}, {0x0B, 0x0D, 0x09, 0x0E}}};

constexpr int Nb = 4; // number of columns (32-bit words) in the state

template<int KeySize>
class AES_Encryption {
public:
    static constexpr int Nk = KeySize / 32; // number of 32-bit words in the key
    static constexpr int Nr = Nk + 6;       // number of rounds

    AES_Encryption() noexcept = default;
    ~AES_Encryption() noexcept = default;

    void KeyExpansion(const uint8_t *key, std::vector<std::array<uint8_t, 4>> &roundKeys) {
        roundKeys.resize((Nr + 1) * Nb);
        for (int i = 0; i < Nk; ++i) {
            for (int j = 0; j < 4; ++j) {
    ...
    void AddRoundKey(std::vector<std::array<uint8_t, 4>> &state, const std::vector<std::array<uint8_t, 4>> &roundKey) const {
        for (int i = 0; i < Nb; ++i) {
            for (int j = 0; j < 4; ++j) {
    ...
    void SubBytes(std::vector<std::array<uint8_t, 4>> &state) const {
        for (auto &row : state) {
            for (auto &byte : row) {
                byte = s_box[byte];
            }
    ...
    void InvSubBytes(std::vector<std::array<uint8_t, 4>> &state) const {
        for (auto &row : state) {
            for (auto &byte : row) {
                byte = inv_s_box[byte];
            }
    ...
    void ShiftRows(std::vector<std::array<uint8_t, 4>> &state) const {
        for (int i = 1; i < 4; ++i) {
            std::rotate(state[i].begin(), state[i].begin() + i, state[i].end());
        }
    }

    void InvShiftRows(std::vector<std::array<uint8_t, 4>> &state) const {
        for (int i = 1; i < 4; ++i) {
            std::rotate(state[i].rbegin(), state[i].rbegin() + i, state[i].rend());
        }
    }

    void MixColumns(std::vector<std::array<uint8_t, 4>> &state) const {
        for (int i = 0; i < Nb; ++i) {
     ...
     void InvMixColumns(std::vector<std::array<uint8_t, 4>> &state) const {
        for (int i = 0; i < Nb; ++i) {
            std::array<uint8_t, 4> temp;
            temp[0] = mul14(state[0][i]) ^ mul11(state[1][i]) ^ mul13(state[2][i]) ^ mul9(state[3][i]);
            temp[1] = mul9(state[0][i]) ^ mul14(state[1][i]) ^ mul11(state[2][i]) ^ mul13(state[3][i]);
            temp[2] = mul13(state[0][i]) ^ mul9(state[1][i]) ^ mul14(state[2][i]) ^ mul11(state[3][i]);
            temp[3] = mul11(state[0][i]) ^ mul13(state[1][i]) ^ mul9(state[2][i]) ^ mul14(state[3][i]);
            for (int j = 0; j < 4; ++j) {
                state[j][i] = temp[j];
            }
        }
    ...
    uint8_t mul2(uint8_t x) const { return (x << 1) ^ ((x & 0x80) ? 0x1B : 0x00); }

    uint8_t mul3(uint8_t x) const { return mul2(x) ^ x; }

    uint8_t mul9(uint8_t x) const { return mul2(mul2(mul2(x))) ^ x; }

    uint8_t mul11(uint8_t x) const { return mul2(mul2(mul2(x))) ^ mul2(x) ^ x; }

    uint8_t mul13(uint8_t x) const { return mul2(mul2(mul2(x))) ^ mul2(mul2(x)) ^ x; }

    uint8_t mul14(uint8_t x) const { return mul2(mul2(mul2(x))) ^ mul2(mul2(x)) ^ mul2(x); }

    void Encrypt(const uint8_t *input, uint8_t *output, const uint8_t *key) {
        std::vector<std::array<uint8_t, 4>> state(4);
...
class AES_Encryption_v2 {
public:
    AES_Encryption_v2() = default;

    template<int KeySize>
    std::string Encrypt(const std::string &plaintext, const std::string &key) {
        if (key.size() != KeySize / 8) {
            throw std::invalid_argument("Key must be " + std::to_string(KeySize / 8) + " bytes long for AES-" + std::to_string(KeySize) + ".");
        }

        size_t padded_len = ((plaintext.size() + 15) / 16) * 16;
        std::vector<uint8_t> input(padded_len, 0);
        std::copy(plaintext.begin(), plaintext.end(), input.begin());

        std::vector<uint8_t> encrypted(padded_len);
        AES_Encryption<KeySize> aes;
        aes.Encrypt(input.data(), encrypted.data(), reinterpret_cast<const uint8_t *>(key.data()));

        std::string encrypted_hex;
        for (const auto &byte : encrypted) {
            encrypted_hex += ToHex(byte);
        }
        return encrypted_hex;
    }

    template<int KeySize>
    std::string Decrypt(const std::string &ciphertext_hex, const std::string &key) {
        if (key.size() != KeySize / 8) {
            throw std::invalid_argument("Key must be " + std::to_string(KeySize / 8) + " bytes long for AES-" + std::to_string(KeySize) + ".");
        }

        size_t padded_len = ciphertext_hex.size() / 2;
        std::vector<uint8_t> encrypted = FromHex(ciphertext_hex);
        std::vector<uint8_t> decrypted(padded_len);

        AES_Encryption<KeySize> aes;
        aes.Decrypt(encrypted.data(), decrypted.data(), reinterpret_cast<const uint8_t *>(key.data()));
