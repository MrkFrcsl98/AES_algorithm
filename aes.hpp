...
constexpr int Nk = 4;  // number of 32-bit words in the key
constexpr int Nr = 10; // number of rounds for AES-128

class AES_Encryption_v1 {
public:
  AES_Encryption_v1() noexcept = default;
  ~AES_Encryption_v1() noexcept = default;
...
public:
  ...
  std::string Encrypt(const std::string &plaintext, const std::string &key) {
    if (key.size() != 16) {
      throw std::invalid_argument("Key must be 16 bytes long for AES-128.");
    }
