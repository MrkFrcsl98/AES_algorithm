...
namespace AES {
Based on the content of the `aes.hpp` file and the description, I will create the AES class with all the necessary functionality. Here is the implementation of the AES algorithm within the AES namespace:
...
private:
    static const int Nb = 4;  // Number of columns (32-bit words) comprising the State. For AES, Nb = 4.
    int Nk;  // Number of 32-bit words comprising the Cipher Key. For AES-128, Nk = 4; for AES-192, Nk = 6; for AES-256, Nk = 8.
    int Nr;  // Number of rounds, which is a function of Nk and Nb (which is fixed). For AES-128, Nr = 10; for AES-192, Nr = 12; for AES-256, Nr = 14.

    std::vector<uint8_t> key;  // The cipher key.
    std::vector<std::vector<uint8_t>> roundKeys;  // Array to hold the key schedule.
...
const uint8_t AES::invSbox[256] = {
    /* Inverse S-box values go here */
};

const uint8_t AES::rcon[10] = {
    /* Round constant values go here */
};
...
void AES::addRoundKey(std::vector<uint8_t>& state, int round) {
    // AddRoundKey transformation implementation
}

void AES::subBytes(std::vector<uint8_t>& state) {
    // SubBytes transformation implementation
}

void AES::invSubBytes(std::vector<uint8_t>& state) {
    // Inverse SubBytes transformation implementation
}

void AES::shiftRows(std::vector<uint8_t>& state) {
    // ShiftRows transformation implementation
}

void AES::invShiftRows(std::vector<uint8_t>& state) {
    // Inverse ShiftRows transformation implementation
}

void AES::mixColumns(std::vector<uint8_t>& state) {
    // MixColumns transformation implementation
}

void AES::invMixColumns(std::vector<uint8_t>& state) {
    // Inverse MixColumns transformation implementation
}
